  ...
  return (
    <div className='max-w-2xl mx-auto p-20 bg-[linear-gradient(360deg,_#4E7C32_68%,_#F9F3EE_100%)] '>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-4 mt-4 [&_input]:border [&_input]:py-1 [&_input]:px-3'>
           <input className='rounded-xl' type='text' placeholder='Họ tên' {...register("name",{required:true})}/>
           ...
           <input className='rounded-xl' type='text' placeholder='Số điện thoại' {
            ...register("phone",{
           ...
           <input className='rounded-xl' type='text' {...register("repassword",{validate:(value:any)=>watch("password")==value})} placeholder='Nhập lại mật khẩu'/>
           {(errors.repassword)&&<span className='text-red-600 text-[12px]'>Mật khẩu không khớp</span>}
            <div className='flex justify-end'>
            <button className='bg-green-900 text-white py-1 px-4 rounded-xl'>Đăng ký</button>
            </div>
        </form>
    </div>
  )
